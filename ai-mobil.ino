...
void setup() {
    ...
    Serial.println("\n========================================");
    Serial.println("   RESCUELINK v2.5 + MOBILE NOTIFY     ");
    Serial.println("========================================");
...
void loop() {
    ...
    // --- 1. ALARM YÖNETİMİ (BUZZER VE LED) ---
    if (deprem_modu_aktif) {
        if (su_an < deprem_modu_bitis) {
            digitalWrite(LED_PIN, (millis() / 150) % 2); // Hızlı flaş
            digitalWrite(BUZZER_PIN, (millis() / 300) % 2); // Kesik ses
        } else {
    ...
    if (su_an >= next_sampling_tick) {
        ...
        if (feature_index >= EI_CLASSIFIER_DSP_INPUT_FRAME_SIZE) {
            ...
            // --- YENİ EŞİK VE BİLDİRİM MANTIĞI ---
            if (ai_sonucu > 0.70) {
                Serial.println("DURUM: 🚨 DEPREM VAR!");
                
                // Mobil Bildirim Gönder
                if (deviceConnected) {
                    pTxCharacteristic->setValue("DEPREM OLUYOR!");
                ...
                // Mobil Bildirim Gönder (Sadece deprem modu aktif değilse gönderir)
                if (deviceConnected && !deprem_modu_aktif) {
                    pTxCharacteristic->setValue("Deprem olabilir bildirimi.");
                    pTxCharacteristic->notify();
                }
            } 
            else if (deprem_modu_aktif) {
                Serial.println("DURUM: 🚨 DEPREM MODU DEVAM EDIYOR");
            }
